...
echo "RunCVM-FC init: Checking for Systemd..."
echo "RunCVM-FC init:   Detailed check:"
ls -la /usr/lib/systemd/systemd 2>/dev/null || echo "  /usr/lib/systemd/systemd not found"
ls -la /lib/systemd/systemd 2>/dev/null || echo "  /lib/systemd/systemd not found"
ls -la /sbin/init 2>/dev/null || echo "  /sbin/init not found"
echo "  PATH=$PATH"
echo "RunCVM-FC init:   SYSTEMD_BIN='$SYSTEMD_BIN'"
echo "RunCVM-FC init:   RUNCVM_SYSTEMD='$RUNCVM_SYSTEMD'"
...
echo "RunCVM-FC init:   SHOULD_RUN_SYSTEMD='$SHOULD_RUN_SYSTEMD'"
...
DBUNITEOF
  ...
  # Add 50% overhead + 2048MB for ext4 metadata and runtime data (logs, docker images)
  local rootfs_size=$(( (staged_size * 3 / 2) + 2048 ))
  [ "$rootfs_size" -lt 2048 ] && rootfs_size=2048
